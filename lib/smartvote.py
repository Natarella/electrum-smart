...
class MasternodePing(object):
    ...
    @classmethod
    def deserialize(cls, vds, protocol_version=90033):
        if protocol_version <= 90033:
            vin = parse_input(vds)
        else:
    ...
    def serialize(self, vds=None):
        if not vds:
            vds = BCDataStream()
        if self.protocol_version <= 90033:
...
class MasternodeAnnounce(object):
    ...
    def __init__(self, alias='', vin=None, addr=NetworkAddress(),
                 ...
                 protocol_version=90033, last_ping=MasternodePing(),
                 announced=False):
        self.alias = alias
        if vin is None:
            vin = {'prevout_hash':'', 'prevout_n': 0, 'scriptSig': '', 'sequence':0xffffffff}
